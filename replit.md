...
- **Canonical Domain Correlation (2026-01-14/2026-01-15):** The `canonical_domain` field exists in IdP and CMDB records but is **NOT used for reconciliation correlation** to match AOD's behavior. AOD doesn't use `canonical_domain` for CMDB/IdP correlation - it only uses `external_ref` and name matching. Farm must match AOD's correlation logic to avoid false positive discrepancies. The field is kept in the generator for future AOD alignment if/when AOD adds canonical_domain support.
