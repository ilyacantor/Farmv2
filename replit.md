...
-   **NLQ Module (Business Data Ground Truth):** **Manifest-driven only** — no self-directed generation. AAM dispatches JobManifests via `POST /api/farm/manifest-intake`, Farm generates data using 8 source system generators (Salesforce, NetSuite, Chargebee, Workday, Zendesk, Jira, Datadog, AWS Cost) anchored by a financial model engine, computes ground truth manifests (v2.0), and persists both data and ground truth to the `ground_truth_manifests` DB table. The NLQ tab is a **read-only dashboard** showing run history, provenance chains (AAM run_id, seed, source systems), DCL push results with pipe status badges, source system record counts, and ground truth metrics with dimensional breakdowns. Server-side verification endpoint `POST /api/business-data/verify/{run_id}` scores actuals against ground truth per-metric with PASS/DEGRADED/FAIL verdicts. DCL readback endpoint stubbed at `POST /verify/{run_id}/dcl-readback` (501 until readback contract provided). The self-directed `POST /api/business-data/generate` endpoint has been removed — Farm does not generate NLQ data without an AAM manifest.
...
-   Manifest-driven mode uses AAM's `pipe_id` (production path). Self-directed mode has been removed.
